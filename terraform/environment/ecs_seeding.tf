...
locals {
  seeding_app = <<EOF
  {
    ...
    "secrets": [
      { "name": "OPG_LPA_POSTGRES_USERNAME", "valueFrom": "${data.aws_secretsmanager_secret.api_rds_username.arn}" },
      { "name": "OPG_LPA_POSTGRES_PASSWORD", "valueFrom": "${data.aws_secretsmanager_secret.api_rds_password.arn}" }
    ],
    "environment": [
