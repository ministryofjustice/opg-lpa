...
resource "aws_ecs_service" "front" {
  ...
  desired_count   = local.account.front_autoscaling_minimum
  launch_type     = "FARGATE"
...
locals {
  ...
  front_app = <<EOF
  {
    ...
    "environment": [
      ...
      {"name": "OPG_NGINX_SERVER_NAMES", "value": "${local.dns_namespace_env}${local.account.front_dns} localhost 127.0.0.1"},
