...
resource "aws_vpc_security_group_egress_rule" "aurora_migration_db_egress" {
  for_each = local.network.allow_all_egress ? toset([]) : toset(compact([local.dms_source_security_group_id, local.dms_target_security_group_id]))
...
resource "aws_vpc_security_group_ingress_rule" "database_from_aurora_migration_ingress" {
  for_each = toset(compact([local.dms_source_security_group_id, local.dms_target_security_group_id]))
