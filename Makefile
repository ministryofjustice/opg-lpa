...
cypress-gui-local:
	docker build -f ./cypress/Dockerfile  -t cypress:latest .; \
	docker run -it -e "DISPLAY=${MYIP}:0" -e "CYPRESS_VIDEO=true" -e "CYPRESS_RUN_A11Y_TESTS=false" -e "CYPRESS_baseUrl=https://localhost:7002"  -v ${PWD}/cypress:/app/cypress --entrypoint cypress --network="host" --rm cypress:latest open --project /app
