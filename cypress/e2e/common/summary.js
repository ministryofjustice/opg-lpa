  ...
  rawTable.forEach((row) => {
    ...
    if (row[2] != '') {
      cy.get('@lpaId').then((lpaId) => {
        cy.get('[data-cy=cya-change]')
          .eq(changeCounter)
          .invoke('attr', 'href')
          .should('contain', '/lpa/' + lpaId + '/' + row[2]);
        changeCounter++;
      });
    }
    counter++;
...
function checkAddress(addrString, addressCounter) {
  ...
  if (addrArray.length > 0) {
    cy.get('[data-cy=streetAddress]')
      .eq(addressCounter)
      .should('contain', addrArray[0].trim());
  }
  if (addrArray.length > 1) {
    cy.get('[data-cy=addressLocality]')
      .eq(addressCounter)
      .should('contain', addrArray[1].trim());
  }
  if (addrArray.length > 2) {
    cy.get('[data-cy=addressRegion]')
      .eq(addressCounter)
      .should('contain', addrArray[2].trim());
  }
  if (addrArray.length > 3) {
    cy.get('[data-cy=postalCode]')
      .eq(addressCounter)
      .should('contain', addrArray[3].trim());
  }
}
