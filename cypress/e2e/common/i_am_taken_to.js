...
Then(`I should be on admin page matching {string}`, (url) => {
  cy.url().should('to.match', new RegExp((Cypress.env('adminUrl') + url).replaceAll('/', '\\/')));
});
...
Then(`I am taken to the life sustaining page`, () => {
  ...
  checkAccordionHeaderContains(
    'Who does the donor want to make decisions about life-sustaining treatment?',
  );
});
...
Then(`I am taken to the fee reduction page`, () => {
  ...
  checkAccordionHeaderContains(
    'Does the donor want to apply to pay a reduced fee?',
  );
});
...
Then(
  `I am taken to the certificate provider page for the test fixture lpa`,
  () => {
    checkOnPageWithPath('certificate-provider');
  },
);
