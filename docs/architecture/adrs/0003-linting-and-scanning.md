...
Note that github actions for code are currently focused on security scans _only_.
...
- Linting PHP code whenever it changes adds overhead to simple edits. For example,
  ...
  triggers the linter to check _all_ of those lines. This may result in the linter
