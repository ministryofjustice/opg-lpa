...
def test_get_queue_url():
    expected = 'http://foo/'
    session_mock.get_queue_url = \
        MagicMock(return_value={'QueueUrl': expected})
...
def test_get_queue_url_exception():
    session_mock.get_queue_url = \
        MagicMock(side_effect=botocore.exceptions.ClientError({}, ''))
    assert(client.get_queue_url() == None)
    session_mock.get_queue_url.assert_called_once_with(QueueName='foo')

def test_queue_exists():
    session_mock.get_queue_url = \
        MagicMock(return_value={'QueueUrl': 'http://foo/'})
