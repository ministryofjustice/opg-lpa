...
class CheckoutController extends AbstractLpaController
{
    ...
    public function payAction()
    {
        ...
        $lpa->payment->gatewayReference = $payment->offsetGet('payment_id');
    ...
    public function payResponseAction()
    {
        ...
        if (!$paymentResponse->isSuccess()) {
            ...
            if ($paymentResponse->offsetGet('state')->offsetGet('code') == 'P0030') {
        ...
        $lpa->payment->reference = $paymentResponse->offsetGet('reference');
        $lpa->payment->date = new \DateTime();
        $lpa->payment->email = new EmailAddress(['address' => strtolower($paymentResponse->offsetGet('email'))]);
