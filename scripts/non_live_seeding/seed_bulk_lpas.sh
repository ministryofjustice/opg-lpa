#!/usr/bin/env sh
# Usage:
#   ./seed_bulk_lpas.sh <user_id> [count]
#
...
if [ -n "$OPG_LPA_POSTGRES_HOSTNAME" ]; then
    echo "Seeding ${COUNT} LPAs for user ${USER_ID} on ${OPG_LPA_POSTGRES_HOSTNAME}..."
    ...
    echo "Seeding ${COUNT} LPAs for user ${USER_ID} on local docker-compose postgres..."
...
echo "Done. Sign in with ${USER_ID}@example.com and Pass1234."
