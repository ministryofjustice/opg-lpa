...
abstract class AbstractLp1 extends AbstractIndividualPdf
{
        ...
        for ($i = 0; $i < self::MAX_ATTORNEYS_SECTION_2; $i++) {
            ...
            if (array_key_exists($i, $primaryAttorneys)) {
                ...
                if ($primaryAttorney instanceof TrustCorporation) {
                    ...
                    $this->setData(
                        'lpa-document-primaryAttorneys-' . $i . '-dob-date-day',
                        $primaryAttorney->dob->date->format('d')
                    );
                    $this->setData(
    ...
    private function populatePageSeven(array $peopleToNotify)
    {
        ...
        for ($i = 0; $i < self::MAX_PEOPLE_TO_NOTIFY_SECTION_6; $i++) {
            ...
            if (array_key_exists($i, $peopleToNotify)) {
                ...
                $this->setData(
                    'lpa-document-peopleToNotify-' . $i . '-name-title',
                    $personToNotify->name->title
                );
                $this->setData(
                    'lpa-document-peopleToNotify-' . $i . '-name-first',
                    $personToNotify->name->first
                );
                $this->setData(
                    'lpa-document-peopleToNotify-' . $i . '-name-last',
                    $personToNotify->name->last
                );
                $this->setData(
    ...
    private function populatePageTwelveThirteenFourteenFifteen(Lpa $lpa, $pageIteration = 0)
    {
        ...
        if ($attorney instanceof Human) {
            // Determine which index key ref to use - for the first
            // MAX_ATTORNEYS_PER_PAGE_SECTION_11 number of pages iterate up
        ...
        // If there is another attorney available, or we are still
        // populating the first MAX_ATTORNEYS_PER_PAGE_SECTION_11 number of pages, trigger again
        $pageIteration++;
     ...
     * in by creating a new copy of the PDF
     *
     * @param $stampFile
     * @param $pageNumber
     * @param bool $stampInsertedPages
     */
    private function stampPageWith($stampFile, $pageNumber, $stampInsertedPages = true)
    {
        ...
        if ($stampInsertedPages) {
            // Determine how many pages of the stamped PDF to use
            // by inspecting the shifted position of the next page (if available)
            $nextPage = $pageNumber + 1;

            // If the current page is the last page then set the number of
            // pages to null so the code below knows to use all trailing pages to the end of the document
