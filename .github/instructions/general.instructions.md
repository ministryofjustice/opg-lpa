...
- nginx (`front-web`, `admin-web`, and their SSL-terminating counterparts `front-ssl`, `admin-ssl`) resolves its upstream hostname (`fastcgi_pass`/`proxy_pass`) ONCE at nginx startup - no dynamic `resolver` directive. If you recreate `front-app` (new container IP), `front-web` (and therefore `front-ssl`, since it proxies to front-web) will keep pointing at the dead IP until restarted. The same applies to `api-web`/`api-app` - e.g. `make dc-reseed` (`docker compose run --rm seeding`) has `depends_on: api-app: condition: service_started`, and Compose recreating/restarting `api-app` to satisfy that dependency will leave `api-web` pointing at the old dead IP ("Host is unreachable" in `api-web` logs) until `api-web` is restarted. `dc-reseed` runs `dc-restart-web` afterwards for exactly this reason - don't remove it. `dc-up`/`dc-up-debug` also run `dc-restart-web` after `docker compose up -d`, since re-running either against an already-running stack (e.g. after pulling new code) only recreates the app containers whose image changed, leaving the already-running web/ssl containers stale in the same way.
- `docker compose restart` does NOT respect `depends_on` ordering the way `docker compose up` does - restarting an nginx container (`front-ssl`/`admin-ssl`) at the same time as the plain web container it proxies to (`front-web`/`admin-web`) can race, with the ssl container's nginx starting before the web container is DNS-resolvable, causing it to crash-exit with `nginx: emerg host not found in upstream "front-web"`. `dc-restart-web` avoids this by restarting `front-web`/`api-web`/`admin-web` first, waiting for `api-web` to respond, and only then restarting `front-ssl`/`admin-ssl` - don't reorder it back to one combined `docker compose restart` call.
- **Use these Makefile targets instead of manual docker compose commands**:
  ...
  - `make dc-restart-web` - restarts front-web/api-web/admin-web, then front-ssl/admin-ssl (in that order - see above), to refresh DNS after any app container is recreated.
  ...
  - `make dc-reseed` - re-runs the non-live seeding scripts; also restarts the web/nginx layer afterwards (via `dc-restart-web`) since the seeding container's dependency on `api-app` can cause it to be recreated with a new IP.
