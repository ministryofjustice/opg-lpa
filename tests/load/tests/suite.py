...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "..", "python-api-client")
)
...
class VisitDashboardBehaviour(SequentialTaskSet):
    ...
    def go_to_dashboard(self):
        with self.client.get(
            "/user/dashboard", catch_response=True, name="3. go to dashboard"
        ) as response:
            ...
            if not expected_text in response.text:
                response.failure(
                    f"expected LPA text {expected_text} was not found in response body"
                )
            ...
            if count_lpas > 1:
                response.failure(
                    f"expected one LPA in dashboard but found {count_lpas}"
                )
...
class MakeAnLpaUser(HttpUser):
    ...
    def on_stop(self):
        ...
        if self.lpa_id is not None:
            logging.info(
                f"cleaning up LPA with ID {self.lpa_id} for user {self.username}"
            )
        ...
        if self.user_id is not None:
            ...
            logging.info(
                f"cleaned up user {self.username} with ID {self.user_id}; status: {status}"
            )
