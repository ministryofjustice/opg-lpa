...
casper.test.begin("Checking user can access repeat application page", {
    ...
    test: function(test) {
        ...
        casper.start(basePath + repeatApplicationPath).then(function () {
        	...
        	this.fill('form[name="form-repeat-application"]', {
        	...
        	test.assertExists('a[class="button dialog__button--accept js-dialog-accept"]', 'Found "Confirm and continue" button');

        }).thenClick('a[class="button dialog__button--accept js-dialog-accept"]', function() {

        	test.info('Clicked [Confirm and continue] button');
