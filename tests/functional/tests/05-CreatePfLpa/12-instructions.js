...
casper.test.begin("Checking instructions page", {
    ...
    test: function(test) {
        ...
        casper.start(basePath + instructionPath).then(function () {
        	...
        	test.assertNotVisible('textarea[name="instruction"]', 'Instructions box is not visible');

        	test.assertExists('textarea[name="preference"]', 'Found preferences box');
        	test.assertNotVisible('textarea[name="preference"]', 'Preferences box is not visible');
		...
		}).waitForSelector('form#form-preferences-and-instructions details', function then () {
