...
casper.test.begin('Checking homepage', {
    ...
    test: function(test) {
        ...
        casper.start(basePath + paths.home).then(function () {
            ...
            var selector = 'a[href="'+ paths.privacy +'"]';

            test.assertVisible( selector, paths.privacy + ' link exists and is visible.');

            // The privacy links opens in a popup, which Casper refuses to handle well!
            // I therefore extract the href, and then access the URL directly to ensure it exists.
