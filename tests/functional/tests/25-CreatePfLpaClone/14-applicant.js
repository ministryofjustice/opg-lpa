...
casper.test.begin("Checking user can select an applicant", {
    ...
    test: function(test) {
        ...
        casper.start(basePath + applicantPath).then(function () {
        	...
        	test.assertExists('input[type="radio"][name="whoIsRegistering"][value="1"]', 'Found attorney radio option');
        ...
        }).thenClick('input[type="radio"][name="whoIsRegistering"][value="1"]', function() {

        	test.info('Selected the attorney as the applicant');
