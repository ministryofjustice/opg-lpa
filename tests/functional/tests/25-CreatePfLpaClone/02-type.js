...
casper.test.begin("Checking user can select an LPA type and create an LPA of the chosen type", {
    ...
    setUp: function(test) {
    	lpaTypePath = paths.lpa_type.replace('\\d+', lpaId);
    	donorPath = paths.donor.replace('\\d+', lpaId);
    },

    tearDown: function(test) {
    	delete lpaTypePath, donorPath;
    },

    test: function(test) {

        casper.start(basePath + lpaTypePath).then(function () {

            test.info('Current URL: ' + this.getCurrentUrl());

            test.assertUrlMatch(new RegExp('^' + basePath + lpaTypePath + '$'), 'Page is on the expected URL.');
        	...
        	test.assertExists('input[type="radio"][name="type"][value="property-and-financial"]', 'Found radio option "Property and financial affairs"');
        	test.assertExists('input[type="radio"][name="type"][value="health-and-welfare"]', 'Found radio option "Health and welfare"');
        	...
        	test.info("Selected LPA type of Property and financial affairs");
        	...
        	// check after form submission, page is landing on donor page.
        	test.assertEquals(basePath + donorPath, this.getCurrentUrl(), 'Now page is on expected URL: ' + this.getCurrentUrl());
